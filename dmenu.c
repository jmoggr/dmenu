...
/* enums */
enum { SchemeNorm, SchemeSel, SchemeOut, SchemeMarker, SchemeLast }; /* color schemes */
...
drawmenu(void)
{
	...
	if (lines > 0) {
		...
		if (curr && curr->left && curr != prev)
			drawmarker(pageupmarker, x, y += bh, npagebefore);

		for (item = curr; item != next; item = item->right)
			drawitem(item, x, y += bh, mw - x);

		if (next && next->right)
			drawmarker(pagedownmarker, x, y += bh, npageafter);
	} else if (matches) {
...
setup(void)
{
	...
	/* create page markers */
	for (i = 0; i < ((mw - promptw)/2)/TEXTW(pagemarker2) + 1; i++) {
		strcat(pageupmarker, pagemarker1);
		strcat(pagedownmarker, pagemarker2);
	}
	for (i = 0; i < ((mw - promptw)/2)/TEXTW(pagemarker1) + 1; i++) {
